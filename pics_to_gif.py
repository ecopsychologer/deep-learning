...
def create_gif(frame_duration_ms=config.FRAME_DURATION, max_epochs=300, image_folder="./logs/", output_folder="./results/", output_name="epochs_0_to_", output_ext=".gif"):
    ...
    for epoch in range(1, latest_img_number + 1):
        if (epoch % 50) == 0:
            ...
            start_time = time.time()
            print(f"{epoch} epochs in {duration:.2f} seconds")
    ...
    if images:
        ...
        print(f"GIF successfully created from images through epoch {latest_img_number} and saved to {output_path}{str(epoch)} in time {total_time:.2f}")
    else:
